...
package io.undertow.connector.xnio.server.http;
...
import io.undertow.connector.xnio.server.http2.Http2ReceiveListener;
...
final class HttpReadListener implements ChannelListener<ConduitStreamSourceChannel>, Runnable {
    ...
    private boolean handleHttp2PriorKnowledge(PooledBuffer pooled, HttpServerExchange httpServerExchange) throws IOException {
    ...
    private void handleHttp2PriorKnowledge(final StreamConnection connection, final HttpServerConnection serverConnection, PooledBuffer readData) throws IOException {
        ...
        if(readData.buffer().hasRemaining()) {
            while (readData.buffer().hasRemaining() && buffer.hasRemaining()) {
                buffer.put(readData.buffer().get());
            }
        }
        final PooledBuffer extraData;
        if(readData.buffer().hasRemaining()) {
            extraData = readData;
        } else {
            readData.close();
    ...
    private boolean doHttp2PriRead(StreamConnection connection, ByteBuffer buffer, HttpServerConnection serverConnection, PooledBuffer extraData) throws IOException {
        if(buffer.hasRemaining()) {
