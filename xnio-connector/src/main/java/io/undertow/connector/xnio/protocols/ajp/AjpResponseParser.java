        ...
        switch (this.state) {
            ...
            case READING_PREFIX_CODE: {
                if (!buf.hasRemaining()) {
                    ...
                    this.prefix = prefix;
                    if (prefix == AjpConstants.FRAME_TYPE_END_RESPONSE) {
                        this.state = READING_PERSISTENT_BOOLEAN;
                        break;
                    } else if (prefix == AjpConstants.FRAME_TYPE_SEND_BODY_CHUNK) {
                        this.state = READING_BODY_CHUNK_LENGTH;
                        break;
                    } else if (prefix != AjpConstants.FRAME_TYPE_SEND_HEADERS && prefix != AjpConstants.FRAME_TYPE_REQUEST_BODY_CHUNK) {
                        this.state = DONE;
                        return;
                    }
                }
            ...
            case READING_STATUS_CODE: {
                ...
                IntegerHolder result = parse16BitInteger(buf);
                if (result.readComplete) {
                    if (this.prefix == AjpConstants.FRAME_TYPE_SEND_HEADERS) {
