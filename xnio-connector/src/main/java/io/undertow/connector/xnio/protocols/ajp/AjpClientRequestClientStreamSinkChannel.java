...
package io.undertow.connector.xnio.protocols.ajp;
...
public class AjpClientRequestClientStreamSinkChannel extends AbstractAjpClientStreamSinkChannel {
    ...
    private SendFrameHeader createFrameHeaderImpl() {
        ...
        if (!firstFrameWritten) {
            ...
            AjpUtils.putString(buffer, AjpUtils.notNull(attachable.getAttachment(ProxiedRequestAttachments.REMOTE_ADDRESS)));
            AjpUtils.putString(buffer, AjpUtils.notNull(attachable.getAttachment(ProxiedRequestAttachments.REMOTE_HOST)));
            AjpUtils.putString(buffer, AjpUtils.notNull(attachable.getAttachment(ProxiedRequestAttachments.SERVER_NAME)));
            AjpUtils.putInt(buffer, AjpUtils.notNull(attachable.getAttachment(ProxiedRequestAttachments.SERVER_PORT)));
            buffer.put((byte) (AjpUtils.notNull(attachable.getAttachment(ProxiedRequestAttachments.IS_SSL)) ? 1 : 0));
