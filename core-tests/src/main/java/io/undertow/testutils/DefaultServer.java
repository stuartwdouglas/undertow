...
import io.undertow.connector.xnio.server.http2.Http2UpgradeHandler;
import io.undertow.connector.xnio.server.spdy.SpdyOpenListener;
import io.undertow.connector.xnio.server.spdy.SpdyPlainOpenListener;
...
public class DefaultServer extends BlockJUnit4ClassRunner {
    ...
    public static final DebuggingSlicePool SSL_BUFFER_POOL = new DebuggingSlicePool(new DefaultByteBufferPool(true, 17 * 1024, 17 * 1024 * 128, 3));
        ...
        if (first) {
            ...
            try {
                ...
                if (ajp) {
                    ...
                    openListener = new SpdyOpenListener(getBufferPool(), new DebuggingSlicePool(new DefaultByteBufferPool(false, BUFFER_SIZE, 10, 1)), OptionMap.create(UndertowOptions.ENABLE_SPDY, true));
                    ...
                    openListener = new Http2OpenListener(getBufferPool(), OptionMap.create(UndertowOptions.ENABLE_HTTP2, true, UndertowOptions.HTTP2_SETTINGS_ENABLE_PUSH, false));
                    ...
                    openListener = new SpdyPlainOpenListener(getBufferPool(), new DebuggingSlicePool(new DefaultByteBufferPool(false, BUFFER_SIZE, 10, 1)), OptionMap.create(UndertowOptions.ENABLE_SPDY, true));
                    acceptListener = ChannelListeners.openListenerAdapter(wrapOpenListener(openListener));
