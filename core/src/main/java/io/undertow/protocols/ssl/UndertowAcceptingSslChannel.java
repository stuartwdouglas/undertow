...
class UndertowAcceptingSslChannel implements AcceptingChannel<SslConnection> {
    ...
    private final ChannelListener.SimpleSetter<AcceptingChannel<SslConnection>> acceptSetter = new ChannelListener.SimpleSetter<>();
    private final Map<XnioIoThread, Queue<UndertowSslConnection>> connectionQueue = new ConcurrentHashMap<>();
    protected final boolean startTls;
    protected final ByteBufferPool applicationBufferPool;
    private final XnioIoThread engineCreateThread;
    ...
    public UndertowAcceptingSslChannel(final UndertowXnioSsl ssl, final AcceptingChannel<? extends StreamConnection> tcpServer, final OptionMap optionMap, final ByteBufferPool applicationBufferPool, final boolean startTls) {
        ...
        //noinspection ThisEscapedInObjectConstruction
        engineCreateThread = tcpServer.getIoThread();
        tcpServer.getAcceptSetter().set(new ChannelListener<AcceptingChannel<? extends StreamConnection>>() {
            ...
            public void handleEvent(AcceptingChannel<? extends StreamConnection> channel) {
                try {
                    final StreamConnection result = channel.accept();
                    if(result == null) {
                        return;
                    }
                    engineCreateThread.execute(new Runnable() {
                        @Override
                        public void run() {
                            try {
                                UndertowSslConnection connection = doAccept(result);
                                Queue<UndertowSslConnection> queue = connectionQueue.get(connection.getIoThread());
                                if(queue == null) {
                                    connectionQueue.put(connection.getIoThread(), queue = new ConcurrentLinkedDeque<UndertowSslConnection>());
                                }
    ...
    public UndertowSslConnection accept() throws IOException {
        Queue<UndertowSslConnection> queue = connectionQueue.get(Thread.currentThread());
        if(queue != null) {
            return queue.poll();
        }
        return null;
    ...
    protected UndertowSslConnection doAccept(StreamConnection tcpConnection) throws IOException {
