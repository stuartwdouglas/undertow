 ...
 */
public class ServerChunkedStreamSourceConduit extends AbstractStreamSourceConduit<StreamSourceConduit> implements ResetableConduit {
    ...
    protected ServerChunkedStreamSourceConduit(final StreamSourceConduit next, final HttpServerConnection connection, final long maxLength) {
        super(next);
    ...
    public int read(final ByteBuffer dst) throws IOException {
        ...
        Pooled<ByteBuffer> pooled = connection.getBufferPool().allocate();
        ...
        try {
            ...
            if (buf.hasRemaining()) {
                if (connection.getExtraBytes() == null) {
                    ...
                    ByteBuffer existing = connection.getExtraBytes().getResource();
                    //the data has to have come from the extra bytes
                    //so we just adjust the extra bytes position
                    //TODO: we may need to revist this, because if there is another channel in the way then this could stuff things up
                    existing.position(existing.position() - buf.remaining());
                    pooled.free();
                }
            }
