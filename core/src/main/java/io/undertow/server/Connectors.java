...
public class Connectors {
     ...
     */
    public static void ungetRequestBytes(final HttpServerExchange exchange, PooledBuffer... buffers) {
        PooledBuffer[] existing = exchange.getAttachment(HttpServerExchange.BUFFERED_REQUEST_DATA);
        PooledBuffer[] newArray;
        if (existing == null) {
            newArray = new PooledBuffer[buffers.length];
            System.arraycopy(buffers, 0, newArray, 0, buffers.length);
        } else {
            newArray = new PooledBuffer[existing.length + buffers.length];
        ...
        exchange.addExchangeCompleteListener(new ExchangeCompletionListener() {
            ...
            public void exchangeEvent(HttpServerExchange exchange, NextListener nextListener) {
                PooledBuffer[] bufs = exchange.getAttachment(HttpServerExchange.BUFFERED_REQUEST_DATA);
                if (bufs != null) {
    ...
    public static void clearRequestChannel(HttpServerExchange exchange) {
